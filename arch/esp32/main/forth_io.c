...
void forth_uart_set_baud(int uart_num, int bps) { uart_set_baudrate(uart_num, bps); }
...
int next_char_from_flash() { // read source stored code from flash memory
    if (buffer_offset < 0 || buffer_offset >= BUFFER_SIZE) {
        ...
        forth_putchar('.');
        esp_flash_read(NULL, (void *) buffer, source_address, BUFFER_SIZE);
        buffer_offset = 0;
    }
    source_address++;
...
int forth_getchar_nowait() {
   ...
   taskYIELD();
   char buf[1];
   return uart_read_bytes(/*port*/0, buf, /*len*/1, 1000 / portTICK_RATE_MS) != 0 ? check_enter() : buf[0];
}
