...
int map_err(int/*sdk_SpiFlashOpResult*/ code) {
    return code;
    //TODOP: 
    // switch (code) {
...
int forth_flash_erase_sector(int sector) {
    return map_err(0/*sdk_spi_flash_erase_sector((uint16_t)sector)*/);
}

int forth_flash_write(int address, void* buffer, int size) {
    return map_err(0/*sdk_spi_flash_write((uint32_t)address, buffer, (uint32_t)size)*/);
}

int forth_flash_read(int address, void* buffer, int size) {
    return map_err(0/*sdk_spi_flash_read((uint32_t)address, buffer, (uint32_t)size)*/);
}
